...
EXPOSE 8000
# Expose additional port if WORKSPACE_MCP_PORT environment variable is set to a different value
ARG WORKSPACE_MCP_PORT
EXPOSE ${WORKSPACE_MCP_PORT:-8000}

# Health check
HEALTHCHECK --interval=30s --timeout=10s --start-period=30s --retries=3 \
    CMD sh -c 'curl -f http://localhost:${WORKSPACE_MCP_PORT:-8000}/health || exit 1'
